...
def main():
    ...
    with pd.HDFStore(args.result_h5file, 'r') as f:
        ...
        vel_log = f['vel_log'].values

        # print ret_mean_log

        iter_log = np.squeeze(iter_log)
        vel_log = np.squeeze(vel_log)
        c = 'red'
        header("Check Dimension")
        header('iter_log : {}'.format(np.shape(iter_log)))
        header('vel_log : {}'.format(np.shape(vel_log)))

        fig = plt.figure(figsize=(10,8))
        plt.plot(iter_log, vel_log, '-', lw=lw, color=c,

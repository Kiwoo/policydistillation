...
class MlpLatentPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        last_out = obz
        for i in range(num_hid_layers):
            if i == num_hid_layers / 2:
                z_mean = tf.nn.tanh(U.dense(last_out, hid_size, "pol1meanfc%i"%(i+1), weight_init=U.normc_initializer(0.1)))
                z_log_sigma_sq = tf.nn.tanh(U.dense(last_out, hid_size, "pol1sigmafc%i"%(i+1), weight_init=U.normc_initializer(0.1)))
                eps_shape = tf.stack([tf.shape(ob)[0], hid_size])
                eps = 0.01*tf.random_normal( eps_shape, 0, 1, dtype=tf.float32)
                last_out = tf.add(z_mean, tf.multiply(tf.sqrt(tf.exp(z_log_sigma_sq)), eps))
            else:
        ...
        self._latentloss = beta * -0.5 * tf.reduce_sum(1 + z_log_sigma_sq 
                                              - tf.square(z_mean)
        ...
        self._act = U.function([stochastic, ob], [ac, self.vpred, self._latentloss])

    def act(self, stochastic, ob):
        ac1, vpred1, latent_loss=  self._act(stochastic, ob[None])
        return ac1[0], vpred1[0], latent_loss

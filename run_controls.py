...
# import logger
import sys
import trpo_controls
from mlp_control_policy import MlpControlPolicy
from misc_util import set_global_seeds
num_cpu=1
...
def train(env_id_list, num_timesteps, seed):
    ...
    def policy_fn(name, ob_space, ac_space):
        return MlpControlPolicy(name=name, ob_space=env.observation_space, ctrl_sz = len(env_list), ac_space=env.action_space,
            hid_size=32, num_hid_layers=2)
    # env = bench.Monitor(env, osp.join(logger.get_dir(), "%i.monitor.json" % rank))
    
    trpo_controls.learn(env_list = env_list, env_id_list = env_id_list, policy_func = policy_fn, timesteps_per_batch=20000, max_kl=0.01, cg_iters=10, cg_damping=0.1,

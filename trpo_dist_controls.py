...
def learn(env_list, env_id_list, policy_func, 
    ...
    optimgain = surrgain #+ entbonus - latent_loss
...
#########################################3
    ...
    while True:        
        ...
        if iters_so_far % 2 == 0:
            ...
            surrbefore, _,_,_,_, latent_loss, g = compute_lossandgrad_dist(*args)
            # print latent_loss
            # failure("===========")
            ...
            if np.allclose(g, 0):
                ...
                for _ in range(10):
                    ...
                    set_from_flat(thnew)
                    meanlosses = surr, kl, _,_,_, _ = np.array(compute_losses(*args))
        ...
        if iters_so_far % 2 == 1:
            # failure("==C0==========")
            surrbefore, _,_,_,_, latent_loss, g = compute_lossandgrad_task(*args)
            surrbefore = np.array(surrbefore)
            # failure("==C1==========")
            if np.allclose(g, 0):
                print("Got zero gradient. not updating")

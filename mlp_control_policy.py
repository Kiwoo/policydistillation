...
class MlpControlPolicy(object):
    ...
    def _init(self, ob_space, ctrl_sz, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        c_in = U.get_placeholder(name="c_in", dtype=tf.float32, shape=[sequence_length, ctrl_sz])
     
        with tf.variable_scope("obfilter"):
            self.ob_rms = RunningMeanStd(shape=ob_space.shape)

        obz = tf.clip_by_value((ob - self.ob_rms.mean) / self.ob_rms.std, -5.0, 5.0)
        last_out = tf.concat([obz, c_in], axis = 1)
